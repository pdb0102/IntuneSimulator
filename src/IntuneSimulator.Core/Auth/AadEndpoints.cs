...
/// <summary>Maps the simulated Azure AD authority endpoints (instance discovery, OIDC metadata, JWKS, and the client-credentials token endpoint).</summary>
public static class AadEndpoints
{
    /// <summary>Maps the AAD authority endpoints onto the application.</summary>
    public static IEndpointRouteBuilder MapAadEndpoints(this IEndpointRouteBuilder app)
    {
            ...
            SimulatorState state, ClientCredentialValidator validator, TokenSigningKey key) =>
        {
            ...
            // A client_credentials request MUST carry client_id (the simulator is deliberately lenient on
            // WHICH id, focusing on the secret/assertion — but an absent id is a malformed request).
            if (string.IsNullOrEmpty(clientId))
                return SimResults.Json(new { error = "invalid_request", error_description = "client_id is required." }, 400);
