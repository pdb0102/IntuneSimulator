...
## Known limitations
- **Simulator subject-mismatch end-to-end — follow-up.** Driving the IntuneSimulator `/control` canned `SubjectNameMismatch` end-to-end from `scepclient` is not yet wired (the simulator validates Intune; it does not issue the SCEP CertRep). Compliance coverage is not blocked — the client already exercises subject/`badRequest` faults against `scepca`'s fault ladder.
