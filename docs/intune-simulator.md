# IntuneSimulator (SCEPwright suite)

> Part of the **SCEPwright** suite. IntuneSimulator fakes the Intune/AAD/Graph cloud chain so a real SCEP **server** can validate its Intune integration - it is **not** a SCEP CA. See the [suite README](../README.md).
>
> To drive the full loop with `scepclient` as the device (fetch the challenge here, enroll against the real SCEP server, which validates back here - and force validation failures like `SubjectNameMismatch`), see **"Testing a real SCEP server's Intune integration"** in [`scepclient-usage.md`](scepclient-usage.md).
...
> **Auth validation is deliberately lenient (test double).** The token endpoint validates the **secret / client assertion** (it must match `--auth-password` or a configured certificate) and requires a `client_id` to be *present*, but it does **not** bind that `client_id` to `--app-id` - any non-empty client id is accepted. This is intentional for a test double whose job is to exercise your product's SCEP/MSAL flow, not to be an authorization server; don't treat the accepted-any-`client_id` behavior as a bug.
...
- **JSON:** `POST https://localhost:8443/challenge` (empty body or `{}`) - returns JSON:
  ...
  ```
- **Default:** The challenge password is derived from the auth password as `base64("intune-scep:" + authPassword)` (UTF-8). For example, auth password `IntunePassw0rd!` yields the base64 of `intune-scep:IntunePassw0rd!`. Override with `--challenge-password` on startup or via `POST /control` at runtime.
...
**The cursor has three states - note that `reset` rewinds, it does *not* disarm:**

| State | Behavior |
|---|---|
| mode `off` (**default**) | no fault is ever injected - the real clean slate |
| mode `manual`/`auto`, step `0`…`31` | inject that step's failure; **step `0` is the FIRST failure** |
| mode `manual`/`auto`, step `32` (= matrix length) | stepping stays on, but every endpoint succeeds (happy path) |

So `reset` returns the cursor to **step 0 (the first failure)**, not to a no-fault state. To get a server that injects nothing, set mode `off`; to exercise the all-succeed path while still in stepping mode, `setStep` to the matrix length. (This 0-based model is why `reset` → step 0.)

### Manual mode (deterministic - recommended for unit/integration tests)

```bash
# Enable manual mode and jump to step 5 (0-based: the `Step 5` row in FAILURE-FLOW.md;
# reset → step 0; step == matrix length means all endpoints succeed):
