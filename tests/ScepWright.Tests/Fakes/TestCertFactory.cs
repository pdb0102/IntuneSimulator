...
namespace ScepWright.Tests.Fakes;
...
internal static class TestCertFactory {
    public static X509Certificate2 Make(string algorithm, int bc_key_usage, DateTime? not_before = null, DateTime? not_after = null) {
        ...
        nb = not_before ?? DateTime.UtcNow.AddMinutes(-5);
        na = not_after ?? DateTime.UtcNow.AddYears(1);
        ...
        if (algorithm.Equals("ml-kem", StringComparison.OrdinalIgnoreCase)) {
            ...
            return Build(pair.Public, signer, "CN=test-ca", "CN=test-kem", bc_key_usage, nb, na);
        }

        signer = new Asn1SignatureFactory(sig_alg, pair.Private);
        return Build(pair.Public, signer, "CN=test", "CN=test", bc_key_usage, nb, na);
    }
    ...
    private static X509Certificate2 Build(AsymmetricKeyParameter subject_public, ISignatureFactory signer, string issuer_dn, string subject_dn, int bc_key_usage, DateTime not_before, DateTime not_after) {
        ...
        cg.SetNotBefore(not_before);
        cg.SetNotAfter(not_after);
